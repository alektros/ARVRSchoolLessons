	...
	public class SFN_Code : SF_Node_Resizeable {
		...
		public override void DrawInner(Rect r){
			...
			if(isEditing){
				...
				if(Event.current.keyCode == KeyCode.Tab && Event.current.type == EventType.Repaint){
					//Debug.Log("Tab");
				...
				if(hoveringNode){
					...
					bool doubleClicked = Event.current.isMouse && Event.current.type == EventType.Repaint && Event.current.clickCount == 2;
